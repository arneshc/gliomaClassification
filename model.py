...
from group_norm import Groupnormalizer



def grun_block(inp, filters, data_format='channels_first', name=None):
    """
    grun_block(inp, filters, name=None)
    ------------------------------------
    ...
    `inp`: An keras.layers.layer instance, required
        The keras layer just preceding the grun block.
    `filters`: integer, required
        No. of filters to use in the 3D convolutional block. The output
        layer of this grun block will have this many no. of channels.
    ...
    `name`: string, optional
        The name to be given to this grun block. Defaults to None, in which
    ...
    `out`: A keras.layers.Layer instance
        The output of the grun block. Has no. of channels equal to `filters`.
...
def lossgt(e=1e-8):
    ...
    Returns
    -------
    lossgt_(y_true, y_pred): A custom keras loss function
        This function takes as input the predicted and ground labels, uses them
        to calculate the dice loss.

    """
    def lossgt_(y_true, y_pred):
        intersection = K.sum(K.abs(y_true * y_pred), axis=[-3,-2,-1])
...
def VAEloss(inpt_shape, z_mean, z_var, weight_L2=0.1, weight_KL=0.1):
    """
    VAEloss(inpt_shape, z_mean, z_var, weight_L2=0.1, weight_KL=0.1)
    ------------------------------------------------------
    Since keras does not allow custom loss functions to have arguments
    other than the true and predicted labels, this function acts as a wrapper
    that allows us to implement the custom loss used in the paper. This function
    ...
    Returns
    -------
    VAEloss_(y_true, y_pred): A custom keras loss function
        This function takes as input the predicted and ground labels, uses them
        to calculate the L2 and KL loss.

    """
    def VAEloss_(y_true, y_pred):
        c, H, W, D = inpt_shape
        n = c * H * W * D
...
def create_model(inpt_shape=(4, 160, 192, 128), output_channels=3, weight_L2=0.1, weight_KL=0.1, dice_e=1e-8):
    """
    create_model(inpt_shape=(4, 160, 192, 128), output_channels=3, weight_L2=0.1, weight_KL=0.1)
    -------------------------------------------
    ...
    `dice_e`: Float, optional
        ...
        zero and possible gradient explosion. This argument will be passed to lossgt function.
    ...
    """
    c, H, W, D = inpt_shape
    assert len(inpt_shape) == 4, "Input shape must be a 4-tuple"
    ...
    ## grun Block x1 (output filters = 32)
    x1 = grun_block(x, 32, name='x1')
    x = Conv3D(
        filters=32,
        kernel_size=(3, 3, 3),
        strides=2,
    ...
    ## grun Block x2 (output filters = 64)
    x = grun_block(x, 64, name='Enc_64_1')
    x2 = grun_block(x, 64, name='x2')
    x = Conv3D(
        filters=64,
        kernel_size=(3, 3, 3),
        strides=2,
    ...
    ## grun Blocks x2 (output filters = 128)
    x = grun_block(x, 128, name='Enc_128_1')
    x3 = grun_block(x, 128, name='x3')
    x = Conv3D(
        filters=128,
        kernel_size=(3, 3, 3),
        strides=2,
    ...
    ## grun Blocks x4 (output filters = 256)
    x = grun_block(x, 256, name='Enc_256_1')
    x = grun_block(x, 256, name='Enc_256_2')
    x = grun_block(x, 256, name='Enc_256_3')
    x4 = grun_block(x, 256, name='x4')
    ...
    ### grun Block x1 (output filters=128)
    x = Conv3D(
        filters=128,
        kernel_size=(1, 1, 1),
        strides=1,
    ...
    x = grun_block(x, 128, name='Dec_GT_128')

    ### grun Block x1 (output filters=64)
    x = Conv3D(
        filters=64,
        kernel_size=(1, 1, 1),
        strides=1,
    ...
    x = grun_block(x, 64, name='Dec_GT_64')
    ...
    x = grun_block(x, 32, name='Dec_GT_32')
    ...
    x = Groupnormalizer(groups=8, axis=1, name='Dec_VAE_VD_GN')(x4)
    ...
    x = grun_block(x, 128, name='Dec_VAE_128')
    ...
    x = grun_block(x, 64, name='Dec_VAE_64')
    ...
    x = grun_block(x, 32, name='Dec_VAE_32')
    ...
    model.compile(
        adam(lr=1e-4),
        [lossgt(dice_e), VAEloss(inpt_shape, z_mean, z_var, weight_L2=weight_L2, weight_KL=weight_KL)],
        metrics=[dice_coeff]
    )
